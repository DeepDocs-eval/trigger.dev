...
3. **Deploy Your Application**: Deploy your application (e.g., to Vercel), setting an environment variable like `TRIGGER_DEPLOYMENT_VERSION` to the captured task version.
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to Vercel
        run: npx vercel --yes --prod -e TRIGGER_DEPLOYMENT_VERSION=$TRIGGER_VERSION --token $VERCEL_TOKEN
        env:
...
- Deploy to Vercel:
  - The `npx vercel` command deploys the application, setting the `TRIGGER_DEPLOYMENT_VERSION` environment variable to the task version from the previous step.
  - The --prod flag ensures a production deployment, and -e passes the environment variable.
  - The `@trigger.dev/sdk` automatically uses the `TRIGGER_DEPLOYMENT_VERSION` environment variable to trigger the correct version of the tasks.
