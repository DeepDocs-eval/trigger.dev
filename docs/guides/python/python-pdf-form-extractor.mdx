...
<Card
  ...
  href="https://github.com/triggerdotdev/examples/tree/main/python-pdf-form-extractor/"
...
This task uses the `python.runScript` method to run the `extract-pdf-form.py` script with the given PDF URL as an argument.
...
export const processPdfForm = task({
  id: "process-pdf-form",
  run: async (payload: { pdfUrl: string }, { python }) => {
